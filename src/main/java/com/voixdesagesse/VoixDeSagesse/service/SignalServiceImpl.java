...
import com.voixdesagesse.VoixDeSagesse.entity.Report;
...
public class SignalServiceImpl implements SignalService {
    ...
    @Override
    @Transactional
    public SignalDTO processSignal(Long signalId, SignalStatus status, String adminComment, Long adminId) throws ArticlaException, Exception {
        ...
        if (status == SignalStatus.APPROVED) {
            ...
            sendReport(signal.getReportedUserId(), articleName);
        }
    ...
    @Transactional
    public Boolean sendReport(Long id, String articleName) throws Exception {
        User user = userService.getUserById(id);
        String email = user.getEmail();
        MimeMessage mm = mailSender.createMimeMessage();
        MimeMessageHelper message = new MimeMessageHelper(mm, true);
        message.setTo(email);
        message.setSubject("[Avertissement] Votre article a été supprimé suite à des signalements");

        message.setText(Report.getDeletedArticleMessage(user.getNom(), user.getPrenom(), articleName), true);
